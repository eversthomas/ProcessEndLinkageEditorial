...
**Geklärt (kein Bug):** Die Änderungen im Mockup selbst greifen korrekt (per HTTP-Server verifiziert: Open Sans lädt, Radius 4/6/8px, Tabellen-/Zeilen-Struktur wie gewünscht). Dass die echte Redaktionsoberfläche des Moduls weiterhin unverändert aussieht, ist erwartetes Verhalten, kein Fehler — das Mockup unter `projektplan/design/mockup/` ist bislang komplett unabhängig vom eigentlichen Modul-Code (`Router.php`, `src/Ui/*.php`, `editorial.css`); es besteht noch keinerlei Verbindung. Die eigentliche Integration ist ein separater, noch nicht begonnener Schritt (siehe unten).

  Vor der Integration muss geklärt werden, welche Mockup-Stellen durch echte Daten ersetzt werden (Platzhalter-Mapping) — Grundlage dafür ist bereits in `collections()` vorhanden, s. o..

  **Mapping-Analyse (unabhängig, gegen echten Code geprüft):** Von sieben Mockup-Zuständen sind drei bereits im Code vorhanden und benötigen nur Restyling — `dashboard` (`DashboardView.php`), `list` (`ListView.php`), `edit` (`FormRenderer.php`, hat bereits Hauptfläche+Details-Sidebar-Split wie im Mockup). Vier Zustände sind dagegen **funktional neu**, nicht nur Design: `blogEdit` (eigener Rich-Text-Editor mit Toolbar/Tags/Terminierung — im Code nur generisches `schemaNeedsTinyMce()`, kein eigener Editor-Layoutzweig), `tree`/Seiten (sortierbare Drag&Drop-Baumansicht — im Code nicht vorhanden, `NavTree.php` betrifft nur die Menü-Konfiguration), `media` (eigenständige Mediathek mit Grid/Suche/Filter — im Code nur feldinterner Bild-Upload), `calendar` (Monatsraster mit Terminen — im Code nicht vorhanden).

  **Entscheidung getroffen:** Statt eines starren Feldtyp-Umfangs bekommt jedes Template in Setup → Redaktion ein Auswahlfeld "Daten-Art" (Blog, News, Termine, Daten, Seiten, ...; Default für Bestandstemplates: "Daten") — bestimmt, welche Mockup-Ansicht für diesen Inhaltstyp gerendert wird. Medien/Mediathek ist davon unabhängig, kein Template-Attribut, sondern ein eigener, globaler Navigationspunkt.

  **Vereinbarte Reihenfolge:** 1) Daten-Art-Auswahlfeld in Setup → Redaktion ergänzen (klein, überschaubar). 2) Mockup-Design für Daten-Art "Daten" (= Dashboard/Liste/Formular) in bestehende PHP-Views übertragen — echtes Restyling, kein Feature-Bau. 3) Blog/News, Termine, Seiten, Medien jeweils **separat** als eigene, spätere Feature-Projekte planen — dort steckt echte neue Funktionalität hinter der Mockup-Optik, nicht nur Styling; explizit nicht Teil der aktuellen Design-Integration.
